...
export default function Quiz() {
  let questions = [
    ...
    },
    {
      ...
      answerOptions: [
        ...
        {
          answerText:
            " it iss that which can be derived by dividing negative number by zero.",
          isCorrect: false,
        },
        {
          answerText:
        ...
        {
          answerText:
            "you change to a new line when not within a string statement",
          isCorrect: false,
        },
      ],
    },
  ...
  let [questionIndex, setQuestionIndex] = useState(randomIndex);
  let [showScore, setShowScore] = useState(false);
  let [score, setScore] = useState(0);
  let [questionNumber, setQuestionNumber] = useState(0);
  let [listAnswers, setListAnswers] = useState([randomIndex]);

  let handleAnswerOptionClick = (isCorrect) => {
    if (isCorrect) {
      setScore(score + 1);
    }
    // let nextQuestion =  randomIndex;
    if (questionNumber + 1 < questions.length) {
      while (listAnswers.includes(randomIndex)) {
        randomIndex = Math.floor(Math.random() * questions.length);
      }
      setListAnswers([...listAnswers, randomIndex]);
